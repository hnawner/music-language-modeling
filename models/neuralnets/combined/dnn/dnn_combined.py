...
stacked_logits_p1 = fully_connected(drop2, 
...
stacked_logits_r1 = fully_connected(drop2, 
			n_hidden_1_r, 
			activation_fn = tf.nn.elu,
			normalizer_fn = batch_norm, 
			normalizer_params=bn_params, 
...
with tf.Session() as s:
    ...
    for e in range(n_epochs):
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            save_path = saver.save(s, "dnn_comb_save/dnn_combined_best.ckpt")
...
with tf.Session() as s:
        saver.restore(s, "dnn_comb_save/dnn_combined_best.ckpt")
