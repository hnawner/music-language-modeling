...
start = 8 # length of cnn window in notes
...
(X_tr_r, X_te_r, y_tr_r, y_te_r, X_tr_p, X_te_p, y_tr_p, y_te_p), 
        (p_max, p_min) = dnn_utils.setup_rnn(train_path, start, trans)
...
X_p_final, y_p_final, X_r_final, y_r_final = \
        dnn_utils.setup_test(test_path, p_max, p_min, start, trans)
...
stacked_outs_p = tf.reshape(network_p, [-1, 12], name='stacked_outs_p')
...
stacked_logits_p3 = fully_connected(p_drop2, 
			num_outputs = n_hidden_3_p, 
			activation_fn = tf.nn.elu,
			normalizer_fn = batch_norm, 
			normalizer_params=bn_params, 
			scope='dense_p3')

p_drop3 = tf.contrib.layers.dropout(stacked_logits_p3, 
        keep_prob_p, 
        is_training = is_training)

stacked_logits_p4 = fully_connected(p_drop3, 
...
logits_p = tf.reshape(stacked_logits_p4,
...
stacked_outs_r = tf.reshape(network_r, [-1, 12], name='stacked_outs_r')
...
r_drop3 = tf.contrib.layers.dropout(stacked_logits_r3, 
        keep_prob_r, 
        is_training = is_training)

stacked_logits_r4 = fully_connected(r_drop3, 
			n_r_outputs, 
			activation_fn = None,
			normalizer_fn = batch_norm, 
			normalizer_params=bn_params_out, 
...
logits_r = tf.reshape(stacked_logits_r4, 
...
with tf.Session() as s:
    ...
    for e in range(n_epochs):
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            save_path = saver.save(s, "dnn_sep_save/dnn_separate_best.ckpt")
        ...
        if e % 10 == 0: # only run sometimes
            ...
            for b in range(n_batches_tr):
                ...
                X_tr_b_r, y_tr_b_r = next_batch(eval_batch_size, 
                        X_tr_r, y_tr_r, b)
                rhy_len = dnn_utils.pad(X_tr_b_r, y_tr_b_r,
                        n_r_inputs, n_r_outputs)

                # pitch
                X_tr_b_p, y_tr_b_p = next_batch(eval_batch_size, 
                        X_tr_p, y_tr_p, b)
                p_len = dnn_utils.pad(X_tr_b_p, y_tr_b_p, 
                        n_p_inputs, n_p_outputs)
            ...
            for b in range(n_batches_te):
                ...
                X_te_b_r, y_te_b_r = next_batch(eval_batch_size, 
                        X_te_r, y_te_r, b)
                rhy_len = dnn_utils.pad(X_te_b_r, y_te_b_r, 
                        n_r_inputs, n_r_outputs)

                # pitch
                X_te_b_p, y_te_b_p = next_batch(eval_batch_size,
                        X_te_p, y_te_p, b)
                p_len = dnn_utils.pad(X_te_b_p, y_te_b_p, 
                        n_p_inputs, n_p_outputs)
            ...
            print('\n------------ %d ------------' % e)
            print('...rhythm...')
            print('loss -train', (sum(log_tr_r) / train_size), 
                    '-tune', (sum(log_te_r)) / test_size)
            print('acc  -train', (sum(acc_tr_r) / train_size), 
                    '-tune', (sum(acc_te_r)) / test_size)

            print('\n...pitch...')
            print('loss -train', (sum(log_tr_p) / train_size), 
...
with tf.Session() as s:
        saver.restore(s, "dnn_sep_save/dnn_separate_best.ckpt")
        ...
        #FINAL TEST
        rhy_len = dnn_utils.pad(X_r_final, y_r_final,
                            n_r_inputs, n_r_outputs)

        p_len = dnn_utils.pad(X_p_final, y_p_final,
                            n_p_inputs, n_p_outputs)


        out_p, out_r, t_loss_final = s.run([logits_p,logits_r, total_loss], 
                            feed_dict={X_r: X_r_final, y_r: y_r_final, 
                            X_p: X_p_final, y_p: y_p_final, 
                            is_training: False})
