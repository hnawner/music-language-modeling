...
start = 1 # always 1 for rnn model
...
# tr = train, te = test
# r = rhythm, p = pitch
(X_tr_r, X_te_r, y_tr_r, y_te_r, X_tr_p, X_te_p, y_tr_p, y_te_p), (p_max, p_min) = \
        rnn_utils.setup_rnn(train_path, start, trans)
...
X_p_final, y_p_final, X_r_final, y_r_final = \
        rnn_utils.setup_test(test_path, p_max, p_min, start, trans)
...
outs_p, _ = tf.nn.dynamic_rnn(multi_p, X_p, dtype=tf.float32,
...
outs_r, _ = tf.nn.dynamic_rnn(multi_r, X_r, dtype=tf.float32,
    ...
    for e in range(n_epochs):
        ...
        for b in range(n_batches):
            ...
            X_batch_r, y_batch_r = next_batch(batch_size, 
                    X_shuf_r, y_shuf_r, b)
            rhy_len = rnn_utils.pad(X_batch_r, y_batch_r, 
        ...
        rhy_len = rnn_utils.pad(X_te_b_r, y_te_b_r, n_r_inputs, n_r_outputs)

        # pitch
        X_te_b_p, y_te_b_p = next_batch(test_size, X_te_p, y_te_p, 0)
        p_len = rnn_utils.pad(X_te_b_p, y_te_b_p,n_p_inputs, n_p_outputs)
        ...
        if test_loss < best_loss:
            best_loss = test_loss
            save_path = saver.save(s, "rnn_sep_save/rnn_separate_best.ckpt")
        ...
        if e % 10 == 0: # only run sometimes
            ...
            for b in range(n_batches_tr):
                ...
                rhy_len = rnn_utils.pad(X_tr_b_r, y_tr_b_r,
                        n_r_inputs, n_r_outputs)
            ...
            for b in range(n_batches_te):
                ...
                # rhythm
                X_te_b_r, y_te_b_r = next_batch(eval_batch_size, X_te_r, y_te_r, b)
                rhy_len = rnn_utils.pad(X_te_b_r, y_te_b_r, 
                        n_r_inputs, n_r_outputs)

                # pitch
                X_te_b_p, y_te_b_p = next_batch(eval_batch_size, X_te_p, y_te_p, b)
...
with tf.Session() as s:
        saver.restore(s, "rnn_sep_save/rnn_separate_best.ckpt")
        ...
        #FINAL TEST
        rhy_len = rnn_utils.pad(X_r_final, y_r_final,
                            n_r_inputs, n_r_outputs)

        p_len = rnn_utils.pad(X_p_final, y_p_final,
                            n_p_inputs, n_p_outputs)
        ...
        print('total acc  -train', total_acc_tr, '-final test', total_acc_final_te)
