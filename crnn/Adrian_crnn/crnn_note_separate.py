...
# Pitch RNN
cells_p = []
cells_p.append(tf.contrib.rnn.LSTMCell(nu_rnn, activation=tf.tanh))
cells_p.append(tf.contrib.rnn.LSTMCell(nu_rnn, activation=tf.tanh))
multi_p = tf.contrib.rnn.MultiRNNCell(cells_p)
outs_p, _ = tf.nn.dynamic_rnn(multi_p, network_p, dtype=tf.float32, swap_memory=True, scope = "pitch")
...
# Rhythm RNN
cells_r = []
cells_r.append(tf.contrib.rnn.LSTMCell(nu_rnn, activation=tf.tanh))
cells_r.append(tf.contrib.rnn.LSTMCell(nu_rnn, activation=tf.tanh))
multi_r = tf.contrib.rnn.MultiRNNCell(cells_r)
outs_r, _ = tf.nn.dynamic_rnn(multi_r, network_r, dtype=tf.float32, swap_memory=True, scope = "rhythm")
...
train_op_r = optimizer_r.minimize(loss_r)
optimizer_p = tf.train.AdamOptimizer(learning_rate=learn_rate)
train_op_p = optimizer_r.minimize(loss_p)
...
with tf.Session() as s:
    ...
    for e in range(n_epochs):
        ...
        for b in range(n_batches):
            ...
            s.run(train_op_p, feed_dict={X_p: X_batch_p, y_p: y_batch_p})
            s.run(train_op_r, feed_dict={X_r: X_batch_r, y_r: y_batch_r})
        ...
        if e == 64:
            ...
            for b in range(n_batches):
                ...
                acc_p, log_p, out_p, acc_r, log_r, out_r = s.run([accuracy_p, loss_p, logits_p, accuracy_r, loss_r, logits_r], feed_dict={X_r: X_tr_b_r, y_r: y_tr_b_r, X_p: X_tr_b_p, y_p: y_tr_b_p})
            ...
            for b in range(n_batches_te):
                ...
                acc_r, log_r, out_r, acc_p, log_p, out_p = s.run([accuracy_r, loss_r, logits_r,accuracy_p, loss_p, logits_p], feed_dict={X_r: X_te_b_r, y_r: y_te_b_r, X_p: X_te_b_p, y_p: y_te_b_p})
