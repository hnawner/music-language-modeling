...
def loss_fn(logits, Y, net):
    #if net == "rhythm":
    #    scale_factor = 0.5
    #    scale_array = [1, scale_factor, 1]
    #    scale_tens = tf.Variable(scale_array, dtype = tf.float32)
    #    Y = tf.multiply(Y, scale_tens)
    xentropy = Y * tf.log(tf.nn.softmax(logits))
    xentropy = -tf.reduce_sum(xentropy, reduction_indices=2)
    mask = tf.sign(tf.reduce_max(tf.abs(Y), reduction_indices=2))
    xentropy *= mask
...
# loss
loss_rhy = loss_fn(logits, Y, "rhythm")
...
with tf.Session() as s:
    ...
    for e in range(n_epochs):
        ...
        for b in range(n_batches):
            ...
            s.run(train_ops_rhy, feed_dict={X: X_batch_rhy, Y: Y_batch_rhy})
        ...
        if e % 20 == 0:
            ...
            for b in range(n_batches):
                ...
                acc_tr_rhy, log_tr_rhy, logits_tr_rhy = s.run([accuracy_rhy, loss_rhy, logits], 
                        feed_dict={X: X_tr_b_rhy, Y: Y_tr_b_rhy})
            ...
            for b in range(n_batches_te):
                ...
                acc_te_rhy, log_te_rhy, logits_te_rhy = s.run([accuracy_rhy, loss_rhy, logits], 
                        feed_dict={X: X_te_b_rhy, Y: Y_te_b_rhy})
