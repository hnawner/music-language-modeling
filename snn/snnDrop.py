...
class SequentialNNDrop:
    ...
    def construct(self, architecture, act, lossFun, keep_prob, learningRate = 0.01):
        ...
        with tf.name_scope('network'):
            ...
            X_drop = tf.contrib.layers.dropout(self.X, keep_prob, is_training = self.is_training)
            
            z = self.new_layer(X_drop, (architecture[0]), 'layer0', act)
            ...
            self.logits = self.new_layer(z_drop, SequentialNNDrop.nOutputs, 'out')
        ...
        with tf.Session() as s:
            ...
            for epoch in range(nEpochs):
                ...
                accTrain = self.accuracy.eval(feed_dict={self.X:Xshuffle,
                                                         self.y:Yshuffle, self.is_training : False})
                accTest = self.accuracy.eval(feed_dict={self.X:self.testX,
                                                        self.y:self.testY, self.is_training : False})
