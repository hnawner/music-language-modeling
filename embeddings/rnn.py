...
class RNN:
    ...
    def construct(self, architecture, cell_type, learningRate = 0.001):
        ...
        # placeholders
        self.X = tf.placeholder(tf.int32, shape=[self.batchsize, None])
        self.y = tf.placeholder(tf.int32, shape=[self.batchsize, None])
        self.seq_lens = tf.placeholder(tf.int32, [self.batchsize])

        embeddings = np.genfromtxt('embedding_dict.txt')
        ...
        rnn_outputs, rnn_states = tf.nn.dynamic_rnn(multi_layer_cell, rnn_inputs, dtype = tf.int32, sequence_length = self.seq_lens, swap_memory = True)
...
r = RNN('/home/hawner2/reu/musical-forms/mels/folk_major')
r.construct([128], tf.contrib.rnn.LSTMCell)
r.execute(10, 32)
